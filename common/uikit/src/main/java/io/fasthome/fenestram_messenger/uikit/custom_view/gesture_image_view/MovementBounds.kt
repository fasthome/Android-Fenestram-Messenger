...
class MovementBounds(private val settings: GeastureImageSettings) {
    companion object {
    ...
    fun set(state: State): MovementBounds {
        val area = tmpRectF
        GravityUtil.getMovementAreaPosition(settings, tmpRect)
        area.set(tmpRect)
        ...
        GravityUtil.getImagePosition(tmpMatrix, settings, pos)
    ...
    fun restrict(x: Float, y: Float, extraX: Float, extraY: Float, out: PointF) {
        ...
        tmpPointArr[0] = MathUtil.restrict(tmpPointArr[0], bounds.left - extraX, bounds.right + extraX)
        tmpPointArr[1] = MathUtil.restrict(tmpPointArr[1], bounds.top - extraY, bounds.bottom + extraY)

...
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.DEFAULT_3G;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.DEFAULT_AUTO_DEMAND;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.DEFAULT_AUTO_ON;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.DEFAULT_AUTO_ONDEMAND;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.DEFAULT_EDGE;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.DEFAULT_NOTIFY;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.DEFAULT_ON_VPN;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.DEFAULT_POSURL;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.DEFAULT_VPN;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.DEFAULT_WLAN;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.PREF_3G;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.PREF_AUTO_DEMAND;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.PREF_AUTO_ON;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.PREF_AUTO_ONDEMAND;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.PREF_EDGE;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.PREF_NOTIFY;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.PREF_ON_VPN;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.PREF_POSURL;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.PREF_SERVER;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.PREF_SIPRINGTONE;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.PREF_USERNAME;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.PREF_VPN;
import static io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.ui.Settings.PREF_WLAN;
...
import io.fasthome.fenestram_messenger.call_impl.R;
import io.fasthome.fenestram_messenger.call_impl.core.sipdroid.media.Bluetooth;
import io.fasthome.fenestram_messenger.call_impl.core.sipdroid.media.RtpStreamReceiver;
import io.fasthome.fenestram_messenger.call_impl.core.sipdroid.media.RtpStreamSender;
import io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.SipdroidEngine;
import io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.UserAgent;
import io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.phone.Call;
import io.fasthome.fenestram_messenger.call_impl.core.sipdroid.sipua.phone.Connection;
import io.fasthome.fenestram_messenger.call_impl.core.zoolu.sip.provider.SipProvider;
...
//			        mContext.sendBroadcast(new Intent(Intent.ACTION_CLOSE_SYSTEM_DIALOGS));
                ...
                if ((pstn_state == null || pstn_state.equals("IDLE")) &&
                        PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(PREF_AUTO_ON, DEFAULT_AUTO_ON) &&
                    ...
                    if (am.getStreamVolume(AudioManager.STREAM_RING) > 0) {
                        String sUriSipRingtone = PreferenceManager.getDefaultSharedPreferences(mContext).getString(PREF_SIPRINGTONE,
    ...
    public static void onText(int type,String text,int mInCallResId,long base) {
        ...
        if (text != null) {
            ...
            if (type == MISSED_CALL_NOTIFICATION) {
                    ...
                    if (PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).getBoolean(PREF_NOTIFY, DEFAULT_NOTIFY)) {
    ...
    static void updateAutoAnswer() {
        if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(PREF_AUTO_ONDEMAND, DEFAULT_AUTO_ONDEMAND) &&
            Sipdroid.on(mContext)) {
            if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(PREF_AUTO_DEMAND, DEFAULT_AUTO_DEMAND))
                updateAutoAnswer(1);
            else
                updateAutoAnswer(0);
        } else
    ...
    public static void url(final String opt) {
        (new Thread() {
            public void run() {
                try {
                    URL url = new URL(PreferenceManager.getDefaultSharedPreferences(mContext).getString(PREF_POSURL, DEFAULT_POSURL)+
                            "?"+opt);
                    BufferedReader in;
    ...
    static boolean on_vpn() {
        return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(PREF_ON_VPN, DEFAULT_ON_VPN);
    }

    static void on_vpn(boolean enable) {
        Editor edit = PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).edit();

        edit.putBoolean(PREF_ON_VPN,enable);
        edit.commit();
    }
    ...
    public static boolean isFast(int i) {
        ...
        if (PreferenceManager.getDefaultSharedPreferences(mContext).getString(PREF_USERNAME+(i!=0?i:""),"").equals("") ||
                PreferenceManager.getDefaultSharedPreferences(mContext).getString(PREF_SERVER+(i!=0?i:""),"").equals(""))
            return false;
        if (wi != null) {
            ...
            if (wi.getIpAddress() != 0 && (WifiInfo.getDetailedStateOf(wi.getSupplicantState()) == DetailedState.OBTAINING_IPADDR
                ...
                on_wlan = true;
                if (!on_vpn())
                    return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(PREF_WLAN+(i!=0?i:""), DEFAULT_WLAN);
                else
                    return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(PREF_VPN+(i!=0?i:""), DEFAULT_VPN);
            }
        }
        on_wlan = false;
        return isFastGSM(i);
    ...
    static boolean isFastGSM(int i) {
        ...
        if (nt >= TelephonyManager.NETWORK_TYPE_UMTS)
            return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(PREF_3G+(i!=0?i:""), DEFAULT_3G);
        if (nt == TelephonyManager.NETWORK_TYPE_EDGE)
               return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(PREF_EDGE+(i!=0?i:""), DEFAULT_EDGE);
        return false;
    }

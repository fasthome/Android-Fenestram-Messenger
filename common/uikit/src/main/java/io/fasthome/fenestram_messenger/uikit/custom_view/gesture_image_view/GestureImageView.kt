...
class GestureImageView @JvmOverloads constructor(
    ...
    private var controller = GestureController(this)
    ...
    fun setSwipeDownListener(action: () -> Unit) {
        controller.setSwipeDownListener(action)
    }
    ...
    override fun setImageDrawable(drawable: Drawable?) {
        ...
        if (drawable == null) {
            ...
            settings.setImageSize(
                settings.viewportWidth.toFloat(),
                settings.viewportHeight.toFloat()
            )
        } else {
            settings.setImageSize(

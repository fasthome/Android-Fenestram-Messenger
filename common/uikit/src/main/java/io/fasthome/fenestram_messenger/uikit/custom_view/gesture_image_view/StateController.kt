...
class StateController internal constructor(private val settings: GeastureImageSettings) {
    companion object {
    ...
    fun updateState(state: State): Boolean {
        return if (isResetRequired) {
            ...
            GravityUtil.getImagePosition(state, settings, tmpRect)
    ...
    fun restrictStateBounds(state: State, prevState: State?, pivotX: Float, pivotY: Float, allowOverzoom: Boolean, restrictRotation: Boolean): Boolean {
        ...
        if (java.lang.Float.isNaN(newPivotX) || java.lang.Float.isNaN(newPivotY)) {
            GravityUtil.getDefaultPivot(settings, tmpPoint)
        ...
        val extraZoom = if (allowOverzoom) GeastureImageSettings.OVERZOOM_FACTOR else 1f

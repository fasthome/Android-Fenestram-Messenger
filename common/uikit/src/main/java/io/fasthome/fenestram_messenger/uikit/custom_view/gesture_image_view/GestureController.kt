...
class GestureController(private val targetView: View) : View.OnTouchListener {
    companion object {
        // перефокусировать изображение при перетаскивании, если палец долго не двигался
        private const val STATE_CHANGE_IDLE_DURATION = 200L
    ...
    private var defZoomToClose = 0f

    private var onSwipeDownListener: () -> Unit = { }
    ...
    val settings: GeastureImageSettings
    val state = State()
    val stateController: StateController

    init {
        val context = targetView.context
        settings = GeastureImageSettings()
    ...
    fun setSwipeDownListener(action: () -> Unit) {
        onSwipeDownListener = action
    }
    ...
    private fun animateStateTo(endState: State?, keepInBounds: Boolean = true) {
        ...
        if (keepInBounds) {
            endStateRestricted =
                stateController.restrictStateBoundsCopy(endState, prevState, pivotX, pivotY)
        }
        ...
        if (!java.lang.Float.isNaN(pivotX) && !java.lang.Float.isNaN(pivotY)) {
            ...
            MathUtil.computeNewPosition(tmpPointArr, stateStart, stateEnd)
    ...
    private fun onTouchInternal(view: View, event: MotionEvent): Boolean {
        ...
        if (isRestrictZoomRequested || isRestrictRotationRequested) {
            ...
            val restrictedState =
                stateController.restrictStateBoundsCopy(state, prevState, pivotX, pivotY)
    ...
    private fun shouldDisallowInterceptTouch(event: MotionEvent): Boolean {
        when (event.actionMasked) {
            MotionEvent.ACTION_DOWN, MotionEvent.ACTION_MOVE -> {
                ...
                val isPannable =
                    State.compare(tmpRectF.width()) > 0 || State.compare(tmpRectF.height()) > 0
                if (isPannable) {
                    return true
                }
            }
    ...
    private fun onDown(event: MotionEvent): Boolean {
        ...
        if (defZoomToClose == 0f) defZoomToClose = state.zoom
    ...
    private fun onUpOrCancel(event: MotionEvent) {
        ...
        if (!mIgnoreCloseDown && (abs(diffY) > abs(diffX)) && (diffY < -mCloseDownThreshold) && downGestureDuration < 350) {
            if (state.zoom in defZoomToClose - .03f..defZoomToClose + .03f) {
    ...
    private fun onScroll(e1: MotionEvent, e2: MotionEvent, dx: Float, dy: Float): Boolean {
        ...
        if (!isScrollDetected) {
            isScrollDetected =
                Math.abs(e2.x - e1.x) > touchSlop || Math.abs(e2.y - e1.y) > touchSlop
    ...
    private inner class AnimationEngine(val view: View) : Runnable {
        ...
        fun onStep(): Boolean {
            ...
            if (!stateScroller.isFinished) {
                ...
                if (java.lang.Float.isNaN(pivotX) || java.lang.Float.isNaN(pivotY) || java.lang.Float.isNaN(
                        endPivotX
                    ) || java.lang.Float.isNaN(endPivotY)
                ) {
                    MathUtil.interpolate(state, stateStart, stateEnd, factor)
                } else {
    ...
    private inner class InternalGesturesListener : GestureDetector.OnGestureListener,
        ...
        override fun onSingleTapConfirmed(event: MotionEvent) =
            this@GestureController.onSingleTapConfirmed(event)

        override fun onDoubleTap(event: MotionEvent) = false

        override fun onDoubleTapEvent(event: MotionEvent) =
            this@GestureController.onDoubleTapEvent(event)
        ...
        override fun onScroll(
            ...
            distanceX: Float,
            distanceY: Float,
        ) = this@GestureController.onScroll(e1, e2, distanceX, distanceY)
        ...
        override fun onFling(e1: MotionEvent, e2: MotionEvent, velocityX: Float, velocityY: Float) =
            this@GestureController.onFling(velocityX, velocityY)

        override fun onRotate(detector: RotationGestureDetector) =
            this@GestureController.onRotate(detector)

        override fun onRotationBegin(detector: RotationGestureDetector) =
            this@GestureController.onRotationBegin()
        ...
        override fun onScale(detector: ScaleGestureDetector) =
            this@GestureController.onScale(detector)

        override fun onScaleBegin(detector: ScaleGestureDetector) =
            this@GestureController.onScaleBegin()
